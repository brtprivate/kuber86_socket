...
const addBet = async (req, res) => {
   try{
        ...
        if (result) {
            minusAmount(req.body.amount, req.body.user_id, req.body.game_id, user)
...
const getUser = async (user_id) => {
    return await UserModel.findOne({ where: { id: user_id } })
}

const getGame = async (game_id) => {
    return await  GameModel.findOne({ where: { id: game_id }, order: [['id', 'DESC']], attributes: ['id', 'status', 'blast_time'], limit: 1 })
}

// get bet amount
const getBetAmount = async (bet_id, user_id) => {
    return await  BetModel.findOne({ where: { id: bet_id, user_id: user_id } })
}


const getSetting = async (admin_id) => {
    return await setting_model.findOne({ where: { id: admin_id } })
}
...
const redeem = async (req, res) => {
    ...
    await BetModel.update(bet_info, { where: { id: bet_id } })
    ...
    await GameModel.update(game_info, { where: { id: game_id } })
    let user_info = {
...
const minusAmount = (amount, user_id, room_id, user) => {
    let info = {
        wallet: Sequelize.literal('wallet-' + amount),
